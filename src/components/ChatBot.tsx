...
const NotionChatbot: React.FC = () => {
  ...
  const [chatSessions, setChatSessions] = useState<ChatSession[]>(() => [createNewChat()]);
  ...
  // Focus input when chat opens or changes
  useEffect(() => {
    if (isOpen && !isMinimized) {
      inputRef.current?.focus();
    }
  }, [isOpen, isMinimized, activeChatId]);
  ...
  const handleDeleteChat = (chatIdToDelete: string) => {
    ...
    setChatSessions(prev => prev.filter(session => session.id !== chatIdToDelete));
  ...
  const sendMessage = async () => {
    ...
    setChatSessions(prev =>
      prev.map(session =>
        session.id === activeChatId
          ? {
              ...session,
              title: session.messages.length === 1 ? userMessage.content.substring(0, 30) : session.title,
    ...
    setTimeout(() => {
      const responses = [
        "Kshitiz has extensive experience in **machine learning** and **deep learning**, particularly in healthcare analytics and recommendation systems. His work at **NIT Kurukshetra** involved building cross-domain systems using `LLMs` and `BERT embeddings`.",
        "His technical expertise includes **Python**, **SQL**, **TensorFlow**, and **PyTorch**. He's also proficient in cloud platforms like **AWS** and has experience with data visualization tools like **Tableau** and **Power BI**.",
        "Kshitiz has worked on exciting projects including healthcare recommendation systems, conversational AI chatbots, and has research experience in **NLP** and **statistical analysis**. He's currently seeking full-time **Data Science** and **Research** roles.",
      ];
    ...
    return (
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1 space-y-1">
          <div
            className="text-gray-300 text-sm leading-relaxed prose-sm"
            dangerouslySetInnerHTML={{ __html: formattedContent + (displayText.length < message.content.length ? '<span class="animate-pulse">|</span>' : '') }}
          />
        </div>
      </div>
    );
  ...
  const WelcomeScreen = () => (
    <div className="flex flex-col items-center text-center space-y-4 my-8 animate-fade-in">
        <div className="w-20 h-20 rounded-full bg-gradient-to-br from-slate-800 to-slate-700 border border-slate-600 flex items-center justify-center shadow-lg">
            <ChatbotIcon size={40} className="text-white/80" /> {/* Larger icon for welcome */}
        </div>
        <div>
            <h3 className="text-xl font-semibold text-white mb-1">How can I help you today?</h3>
            <p className="text-sm text-gray-400">I know everything about Kshitiz's portfolio</p>
        </div>
        <div className="w-full pt-4 space-y-2">
            {[
              { icon: "📄", text: "Summarize his portfolio", action: "Give me a summary of Kshitiz's background and experience" },
              { icon: "💡", text: "Show key projects", action: "Tell me about Kshitiz's most important projects" }
            ].map((item, index) => (
              <button
                key={index}
                onClick={() => {
        ...
        >
          ...
          <div className={`flex items-center justify-between shrink-0 ${isMinimized ? 'px-4 py-1' : 'px-4 py-2 border-b border-white/10'}`}>
            <div className="relative" ref={dropdownRef}>
              <button 
                onClick={() => !isMinimized && setIsDropdownOpen(!isDropdownOpen)}
                className="flex items-center space-x-1 text-gray-200 hover:text-white transition-colors p-1 rounded-md disabled:pointer-events-none"
                disabled={isMinimized}
              >
                <span className={`font-medium ${isMinimized ? 'text-sm' : 'text-base'}`}>
                  {isMinimized ? 'New Chat...' : (activeChat?.title.substring(0, 20) || 'New Chat')}
                </span>
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-64 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-10 animate-fade-in-fast">
                    <div className="p-1">
                        <button onClick={handleNewChat} className="w-full flex items-center space-x-2 px-3 py-2 text-sm text-left text-gray-300 hover:bg-slate-700 rounded-md">
                            <Plus size={14} />
                            <span>New Chat</span>
                        </button>
                    </div>
                    ...
                    <div className="p-1 max-h-60 overflow-y-auto custom-scrollbar">
                        {chatSessions.map(session => (
                            <div key={session.id} className="group flex items-center justify-between hover:bg-slate-700 rounded-md">
                                <button
                                    onClick={() => { setActiveChatId(session.id); setIsDropdownOpen(false); }}
                                    className={`w-full text-left px-3 py-2 text-sm truncate ${activeChatId === session.id ? 'text-blue-400' : 'text-gray-300'}`}
                                >
                                    {session.title}
                                </button>
                                {chatSessions.length > 1 && (
                                    <button onClick={() => handleDeleteChat(session.id)} className="p-2 text-gray-500 hover:text-red-400 opacity-0 group-hover:opacity-100">
                                        <Trash2 size={14} />
                                    </button>
                                )}
                            </div>
          ...
          {!isMinimized && (
            <div className="flex-1 flex flex-col min-h-0">
              <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6 custom-scrollbar">
                ...
                {activeChat && activeChat.messages.slice(1).map((msg) => (
                  <div key={msg.id}>
                    {msg.role === 'user' ? (
                      <div className="flex justify-end animate-slide-in-right">
                        <div className="max-w-[85%] bg-blue-600 text-white px-4 py-2 rounded-2xl rounded-tr-lg">
                ...
                {isLoading && (
                  <div className="flex items-start space-x-3 animate-slide-in-left">
                    ...
                    <div className="bg-slate-800 px-4 py-3 rounded-2xl rounded-tl-lg">
              ...
              <div className="border-t border-white/10 bg-black/20 p-4">
                  <div className="relative">
                    <textarea
                      ...
                      placeholder="Ask me anything about Kshitiz..."
                      className="w-full pl-4 pr-12 py-2.5 bg-slate-800/80 border border-slate-700 rounded-xl text-gray-200 text-sm placeholder-gray-500 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all duration-200 custom-scrollbar"
                      disabled={isLoading}
                      rows={1}
                    ...
                    <button
                      ...
                      className="absolute right-2.5 top-1/2 -translate-y-1/2 w-8 h-8 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg transition-all duration-200 flex items-center justify-center"
                    >
                      <Send size={14} />
                    </button>
                  </div>
